...
public class AntPathMatcherTest {
	...
	public void extractUriTemplateVariables() {
		...
		expect = new LinkedHashMap<String, String>();
		expect.put("hotel", "1");
		expect.put("booking", "2");
		...
		result = pathMatcher.extractUriTemplateVariables("/{name}.{ext}", "/test.html");
		expect = new LinkedHashMap<String, String>();
		expect.put("name", "test");
		expect.put("ext", "html");
		assertEquals(expect, result);
	}
	...
	public void extractUriTemplateVariablesCustomRegex() {
		...
		result = pathMatcher.extractUriTemplateVariables(
			"{name:[\\w\\.]+}-{version:[\\w\\.]+}.jar", "com.example-1.0.0.jar");
		assertEquals("com.example", result.get("name"));
		assertEquals("1.0.0", result.get("version"));
		
		result = pathMatcher.extractUriTemplateVariables(
				"{name:[\\w\\.]+}-sources-{version:[\\w\\.]+}.jar", "com.example-sources-1.0.0.jar");
		assertEquals("com.example", result.get("name"));
		assertEquals("1.0.0", result.get("version"));
	}
